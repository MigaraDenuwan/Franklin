...
class HatcheryEngine:
    ...
    def register_video(self, vid, path):
        # Allow both local paths and HTTP URLs
        ...
        self.init_state(vid)
        print(f"Registered video: {vid} → {path}")
        return True
    ...
    def process_frame(self, frame, vid, fps):
        ...
        if results and results[0].boxes is not None:
            ...
            ids = r.boxes.id.int().cpu().tolist() if r.boxes.id is not None else list(range(len(boxes)))
        ...
        if self._frame_count[vid] % 100 == 0:
            ...
            print(f" [{vid}] frame={self._frame_count[vid]} "
                  f"species={state['species']} status={state['status']} health={state['health']}")
    ...
    def update_logic(self, vid, f_species, f_status, f_health):
        ...
        if len(m["status_buffer"]) >= CONFIRMATION_WINDOW:
            ...
            # Only call Node for uploaded videos, in a background thread
            if vid.startswith("upload_") and self.node_api_url:
                self._async_node_update(vid, self.states[vid]["species"], final_s, final_h)

    def _async_node_update(self, vid, species, behavior, health):
        """
        Run Node update in a background thread so it never blocks the stream.
        Uses a proper timeout so it actually completes.
        """
        def _post():
            try:
                url = f"{self.node_api_url}/video/{vid}/analysis"
                ...
                if response.status_code == 200:
                    print(f"MongoDB updated for {vid}: {species} - {behavior} - {health}")
                else:
                    print(f"Node update returned {response.status_code} for {vid}")
            except requests.exceptions.Timeout:
                print(f"Node update timed out for {vid}")
            except requests.exceptions.ConnectionError:
                print(f"Node connection refused for {vid} — is Node running?")
            except Exception as e:
                print(f"Node update failed for {vid}: {e}")

        # Fire in background — never blocks the video stream
